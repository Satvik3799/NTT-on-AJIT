...
void ntt_thread(void* args){
    ...
    // PRINTF("[INFO]  :   NTT_THREAD running\n");
}
...
void ntt_top(uint16_t *x, uint16_t *psis){
    ...
    // PRINTF("[INFO]  :   NTT on Even indices - running on a thread\n");
    ...
    // PRINTF("[INFO]  :   NTT on Odd indices - running locally\n");
    ct_ntt(xo, psis);
