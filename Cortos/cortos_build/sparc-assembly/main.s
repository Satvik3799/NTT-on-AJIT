	.file	"main.c"
	.section	".text"
.Ltext0:
	.cfi_sections	.debug_frame
	.common	tc,24,4
	.align 4
	.global set_NTT_Args
	.type	set_NTT_Args, #function
	.proc	020
set_NTT_Args:
.LFB3:
	.file 1 "../main.c"
	.loc 1 29 0
	.cfi_startproc
	save	%sp, -96, %sp
.LCFI0:
	.cfi_window_save
	.cfi_register 15, 31
	.cfi_def_cfa_register 30
	st	%i0, [%fp+68]
	st	%i1, [%fp+72]
	st	%i2, [%fp+76]
	.loc 1 30 0
	ld	[%fp+68], %g1
	ld	[%fp+72], %g2
	st	%g2, [%g1]
	.loc 1 31 0
	ld	[%fp+68], %g1
	ld	[%fp+76], %g2
	st	%g2, [%g1+4]
	.loc 1 32 0
	restore
	jmp	%o7+8
	 nop
	.cfi_endproc
.LFE3:
	.size	set_NTT_Args, .-set_NTT_Args
	.align 4
	.global ntt_thread
	.type	ntt_thread, #function
	.proc	020
ntt_thread:
.LFB4:
	.loc 1 35 0
	.cfi_startproc
	save	%sp, -104, %sp
.LCFI1:
	.cfi_window_save
	.cfi_register 15, 31
	.cfi_def_cfa_register 30
	st	%i0, [%fp+68]
	.loc 1 36 0
	ld	[%fp+68], %g1
	ld	[%g1], %g1
	st	%g1, [%fp-4]
	.loc 1 37 0
	ld	[%fp+68], %g1
	ld	[%g1+4], %g1
	st	%g1, [%fp-8]
	.loc 1 48 0
	ld	[%fp-4], %o0
	ld	[%fp-8], %o1
	call	ct_ntt, 0
	 nop
	.loc 1 51 0
	restore
	jmp	%o7+8
	 nop
	.cfi_endproc
.LFE4:
	.size	ntt_thread, .-ntt_thread
	.align 4
	.global ntt_top
	.type	ntt_top, #function
	.proc	020
ntt_top:
.LFB5:
	.loc 1 53 0
	.cfi_startproc
	save	%sp, -632, %sp
.LCFI2:
	.cfi_window_save
	.cfi_register 15, 31
	.cfi_def_cfa_register 30
	st	%i0, [%fp+68]
	st	%i1, [%fp+72]
	.loc 1 55 0
	sethi	%hi(tc), %g1
	or	%g1, %lo(tc), %g1
	st	%g1, [%fp-8]
	.loc 1 80 0
	stb	%g0, [%fp-1]
	b	.L4
	 nop
.L5:
	.loc 1 81 0 discriminator 2
	ldub	[%fp-1], %g1
	and	%g1, 0xff, %g1
	ldub	[%fp-1], %g2
	and	%g2, 0xff, %g2
	sll	%g2, 2, %g2
	ld	[%fp+68], %g3
	add	%g3, %g2, %g2
	lduh	[%g2], %g2
	add	%g1, %g1, %g1
	add	%fp, %g1, %g1
	sth	%g2, [%g1-272]
	.loc 1 82 0 discriminator 2
	ldub	[%fp-1], %g1
	and	%g1, 0xff, %g1
	ldub	[%fp-1], %g2
	and	%g2, 0xff, %g2
	sll	%g2, 2, %g2
	add	%g2, 2, %g2
	ld	[%fp+68], %g3
	add	%g3, %g2, %g2
	lduh	[%g2], %g2
	add	%g1, %g1, %g1
	add	%fp, %g1, %g1
	sth	%g2, [%g1-528]
	.loc 1 80 0 discriminator 2
	ldub	[%fp-1], %g1
	add	%g1, 1, %g1
	stb	%g1, [%fp-1]
.L4:
	.loc 1 80 0 is_stmt 0 discriminator 1
	ldub	[%fp-1], %g1
	sll	%g1, 24, %g1
	sra	%g1, 24, %g1
	cmp	%g1, 0
	bge	.L5
	 nop
	.loc 1 95 0 is_stmt 1
	add	%fp, -16, %g2
	add	%fp, -272, %g1
	mov	%g2, %o0
	mov	%g1, %o1
	ld	[%fp+72], %o2
	call	set_NTT_Args, 0
	 nop
	.loc 1 96 0
	nop
.L6:
	.loc 1 96 0 is_stmt 0 discriminator 1
	ld	[%fp-8], %g2
	add	%fp, -16, %g1
	mov	%g2, %o0
	sethi	%hi(ntt_thread), %g2
	or	%g2, %lo(ntt_thread), %o1
	mov	%g1, %o2
	call	scheduleChannelJob, 0
	 nop
	mov	%o0, %g1
	cmp	%g1, 0
	bne	.L6
	 nop
	.loc 1 102 0 is_stmt 1
	add	%fp, -528, %g1
	mov	%g1, %o0
	ld	[%fp+72], %o1
	call	ct_ntt, 0
	 nop
	.loc 1 104 0
	st	%g0, [%fp-532]
	.loc 1 105 0
	nop
.L7:
	.loc 1 105 0 is_stmt 0 discriminator 1
	ld	[%fp-8], %g2
	add	%fp, -532, %g1
	mov	%g2, %o0
	mov	%g1, %o1
	call	getChannelResponse, 0
	 nop
	mov	%o0, %g1
	cmp	%g1, 0
	bne	.L7
	 nop
	.loc 1 112 0 is_stmt 1
	restore
	jmp	%o7+8
	 nop
	.cfi_endproc
.LFE5:
	.size	ntt_top, .-ntt_top
	.section	".rodata"
	.align 8
.LC1:
	.asciz	"Init channel.\n"
	.align 8
.LC2:
	.asciz	"initChannel %d: status=%d.\n"
	.section	".text"
	.align 4
	.global setup
	.type	setup, #function
	.proc	020
setup:
.LFB6:
	.loc 1 115 0
	.cfi_startproc
	save	%sp, -96, %sp
.LCFI3:
	.cfi_window_save
	.cfi_register 15, 31
	.cfi_def_cfa_register 30
	.loc 1 116 0
	mov	1, %o0
	mov	0, %o1
	mov	0, %o2
	call	__ajit_serial_configure_via_vmap__, 0
	 nop
	.loc 1 117 0
	sethi	%hi(114688), %g1
	or	%g1, 512, %o0
	sethi	%hi(80000000), %o1
	call	__ajit_serial_set_baudrate_via_vmap__, 0
	 nop
	.loc 1 118 0
	mov	0, %o0
	call	__ajit_serial_set_uart_reset_via_vmap__, 0
	 nop
	.loc 1 120 0
	sethi	%hi(.LC1), %g1
	or	%g1, %lo(.LC1), %o0
	call	cortos_printf, 0
	 nop
	.loc 1 122 0
	sethi	%hi(tc), %g1
	or	%g1, %lo(tc), %o0
	mov	1, %o1
	call	initChannel, 0
	 nop
	.loc 1 124 0
	sethi	%hi(tc), %g1
	or	%g1, %lo(tc), %g1
	ld	[%g1], %g2
	sethi	%hi(tc), %g1
	or	%g1, %lo(tc), %g1
	ld	[%g1+20], %g1
	sethi	%hi(.LC2), %g3
	or	%g3, %lo(.LC2), %o0
	mov	%g2, %o1
	mov	%g1, %o2
	call	cortos_printf, 0
	 nop
	.loc 1 125 0
	restore
	jmp	%o7+8
	 nop
	.cfi_endproc
.LFE6:
	.size	setup, .-setup
	.section	".rodata"
	.align 8
.LC3:
	.asciz	"[INFO]  :   measure on single thread.\n"
	.align 8
.LC4:
	.asciz	"[RESULT]  :   Single-thread NTT\n"
	.global __floatundidf
	.align 8
.LC5:
	.asciz	"[RESULT]  :   Single thread Times: %f %f\n"
	.align 8
.LC6:
	.asciz	"[INFO]  :   Single-thread NTT measure on two threads.\n"
	.align 8
.LC7:
	.asciz	"\n"
	.align 8
.LC8:
	.asciz	"[RESULT]  :   Dual thread Times: %f %f\n"
	.align 8
.LC9:
	.asciz	"[RESULT]  :   Dual-thread NTT\n"
	.align 8
.LC10:
	.asciz	"[INFO]  :   close the channel..\n"
	.align 2
.LC0:
	.half	0
	.half	1
	.half	0
	.half	0
	.half	0
	.half	0
	.half	1
	.half	3327
	.half	0
	.half	3328
	.half	3328
	.half	1
	.half	2
	.half	1
	.half	3328
	.half	1
	.half	1
	.half	0
	.half	3328
	.half	0
	.half	1
	.half	0
	.half	3326
	.half	3328
	.half	1
	.half	3327
	.half	2
	.half	0
	.half	1
	.half	1
	.half	0
	.half	0
	.half	3328
	.half	3328
	.half	1
	.half	3328
	.half	1
	.half	0
	.half	0
	.half	3328
	.half	2
	.half	2
	.half	3327
	.half	1
	.half	2
	.half	0
	.half	0
	.half	1
	.half	0
	.half	0
	.half	0
	.half	0
	.half	1
	.half	3328
	.half	1
	.half	1
	.half	0
	.half	0
	.half	0
	.half	0
	.half	1
	.half	1
	.half	3328
	.half	1
	.half	0
	.half	0
	.half	1
	.half	3328
	.half	0
	.half	0
	.half	2
	.half	0
	.half	0
	.half	0
	.half	3328
	.half	3327
	.half	3327
	.half	0
	.half	3328
	.half	3328
	.half	0
	.half	3327
	.half	1
	.half	3328
	.half	1
	.half	3328
	.half	0
	.half	2
	.half	0
	.half	3327
	.half	1
	.half	0
	.half	1
	.half	1
	.half	0
	.half	0
	.half	3327
	.half	3328
	.half	0
	.half	1
	.half	0
	.half	0
	.half	3328
	.half	0
	.half	0
	.half	3328
	.half	3328
	.half	0
	.half	3328
	.half	3327
	.half	1
	.half	1
	.half	3328
	.half	0
	.half	1
	.half	1
	.half	3328
	.half	0
	.half	3328
	.half	3326
	.half	0
	.half	0
	.half	3328
	.half	0
	.half	2
	.half	3328
	.half	0
	.half	1
	.half	0
	.half	0
	.half	2
	.half	3328
	.half	0
	.half	3328
	.half	3328
	.half	0
	.half	0
	.half	0
	.half	1
	.half	2
	.half	3328
	.half	3327
	.half	1
	.half	0
	.half	2
	.half	2
	.half	2
	.half	3327
	.half	2
	.half	0
	.half	0
	.half	1
	.half	0
	.half	3328
	.half	3328
	.half	0
	.half	1
	.half	0
	.half	0
	.half	0
	.half	3328
	.half	3328
	.half	1
	.half	3
	.half	3328
	.half	1
	.half	3328
	.half	2
	.half	0
	.half	0
	.half	0
	.half	2
	.half	0
	.half	1
	.half	1
	.half	3328
	.half	1
	.half	0
	.half	3328
	.half	3328
	.half	0
	.half	3328
	.half	3328
	.half	1
	.half	3328
	.half	3327
	.half	1
	.half	0
	.half	1
	.half	3328
	.half	1
	.half	3328
	.half	1
	.half	3328
	.half	1
	.half	0
	.half	3327
	.half	3328
	.half	1
	.half	3
	.half	3327
	.half	0
	.half	1
	.half	3327
	.half	3
	.half	0
	.half	1
	.half	1
	.half	1
	.half	1
	.half	3327
	.half	3328
	.half	3328
	.half	3328
	.half	1
	.half	3326
	.half	0
	.half	1
	.half	1
	.half	1
	.half	0
	.half	0
	.half	1
	.half	0
	.half	0
	.half	2
	.half	0
	.half	3328
	.half	3328
	.half	3328
	.half	1
	.half	3
	.half	3328
	.half	0
	.half	3328
	.half	2
	.half	1
	.half	3327
	.half	0
	.half	1
	.half	1
	.half	0
	.half	3327
	.half	3328
	.half	1
	.half	1
	.half	0
	.half	3328
	.half	3328
	.half	3328
	.half	0
	.half	3328
	.half	0
	.half	0
	.half	3328
	.half	0
	.section	".text"
	.align 4
	.global main_00
	.type	main_00, #function
	.proc	014
main_00:
.LFB7:
	.loc 1 128 0
	.cfi_startproc
	save	%sp, -1160, %sp
.LCFI4:
	.cfi_window_save
	.cfi_register 15, 31
	.cfi_def_cfa_register 30
	.loc 1 132 0
	add	%fp, -544, %g2
	mov	512, %g1
	mov	%g2, %o0
	mov	0, %o1
	mov	%g1, %o2
	call	memset, 0
	 nop
	mov	1, %g1
	sth	%g1, [%fp-544]
	mov	1729, %g1
	sth	%g1, [%fp-542]
	mov	2580, %g1
	sth	%g1, [%fp-540]
	mov	3289, %g1
	sth	%g1, [%fp-538]
	mov	2642, %g1
	sth	%g1, [%fp-536]
	mov	630, %g1
	sth	%g1, [%fp-534]
	mov	1897, %g1
	sth	%g1, [%fp-532]
	mov	848, %g1
	sth	%g1, [%fp-530]
	mov	1062, %g1
	sth	%g1, [%fp-528]
	mov	1919, %g1
	sth	%g1, [%fp-526]
	mov	193, %g1
	sth	%g1, [%fp-524]
	mov	797, %g1
	sth	%g1, [%fp-522]
	mov	2786, %g1
	sth	%g1, [%fp-520]
	mov	3260, %g1
	sth	%g1, [%fp-518]
	mov	569, %g1
	sth	%g1, [%fp-516]
	mov	1746, %g1
	sth	%g1, [%fp-514]
	mov	296, %g1
	sth	%g1, [%fp-512]
	mov	2447, %g1
	sth	%g1, [%fp-510]
	mov	1339, %g1
	sth	%g1, [%fp-508]
	mov	1476, %g1
	sth	%g1, [%fp-506]
	mov	3046, %g1
	sth	%g1, [%fp-504]
	mov	56, %g1
	sth	%g1, [%fp-502]
	mov	2240, %g1
	sth	%g1, [%fp-500]
	mov	1333, %g1
	sth	%g1, [%fp-498]
	mov	1426, %g1
	sth	%g1, [%fp-496]
	mov	2094, %g1
	sth	%g1, [%fp-494]
	mov	535, %g1
	sth	%g1, [%fp-492]
	mov	2882, %g1
	sth	%g1, [%fp-490]
	mov	2393, %g1
	sth	%g1, [%fp-488]
	mov	2879, %g1
	sth	%g1, [%fp-486]
	mov	1974, %g1
	sth	%g1, [%fp-484]
	mov	821, %g1
	sth	%g1, [%fp-482]
	mov	289, %g1
	sth	%g1, [%fp-480]
	mov	331, %g1
	sth	%g1, [%fp-478]
	mov	3253, %g1
	sth	%g1, [%fp-476]
	mov	1756, %g1
	sth	%g1, [%fp-474]
	mov	1197, %g1
	sth	%g1, [%fp-472]
	mov	2304, %g1
	sth	%g1, [%fp-470]
	mov	2277, %g1
	sth	%g1, [%fp-468]
	mov	2055, %g1
	sth	%g1, [%fp-466]
	mov	650, %g1
	sth	%g1, [%fp-464]
	mov	1977, %g1
	sth	%g1, [%fp-462]
	mov	2513, %g1
	sth	%g1, [%fp-460]
	mov	632, %g1
	sth	%g1, [%fp-458]
	mov	2865, %g1
	sth	%g1, [%fp-456]
	mov	33, %g1
	sth	%g1, [%fp-454]
	mov	1320, %g1
	sth	%g1, [%fp-452]
	mov	1915, %g1
	sth	%g1, [%fp-450]
	mov	2319, %g1
	sth	%g1, [%fp-448]
	mov	1435, %g1
	sth	%g1, [%fp-446]
	mov	807, %g1
	sth	%g1, [%fp-444]
	mov	452, %g1
	sth	%g1, [%fp-442]
	mov	1438, %g1
	sth	%g1, [%fp-440]
	mov	2868, %g1
	sth	%g1, [%fp-438]
	mov	1534, %g1
	sth	%g1, [%fp-436]
	mov	2402, %g1
	sth	%g1, [%fp-434]
	mov	2647, %g1
	sth	%g1, [%fp-432]
	mov	2617, %g1
	sth	%g1, [%fp-430]
	mov	1481, %g1
	sth	%g1, [%fp-428]
	mov	648, %g1
	sth	%g1, [%fp-426]
	mov	2474, %g1
	sth	%g1, [%fp-424]
	mov	3110, %g1
	sth	%g1, [%fp-422]
	mov	1227, %g1
	sth	%g1, [%fp-420]
	mov	910, %g1
	sth	%g1, [%fp-418]
	mov	17, %g1
	sth	%g1, [%fp-416]
	mov	2761, %g1
	sth	%g1, [%fp-414]
	mov	583, %g1
	sth	%g1, [%fp-412]
	mov	2649, %g1
	sth	%g1, [%fp-410]
	mov	1637, %g1
	sth	%g1, [%fp-408]
	mov	723, %g1
	sth	%g1, [%fp-406]
	mov	2288, %g1
	sth	%g1, [%fp-404]
	mov	1100, %g1
	sth	%g1, [%fp-402]
	mov	1409, %g1
	sth	%g1, [%fp-400]
	mov	2662, %g1
	sth	%g1, [%fp-398]
	mov	3281, %g1
	sth	%g1, [%fp-396]
	mov	233, %g1
	sth	%g1, [%fp-394]
	mov	756, %g1
	sth	%g1, [%fp-392]
	mov	2156, %g1
	sth	%g1, [%fp-390]
	mov	3015, %g1
	sth	%g1, [%fp-388]
	mov	3050, %g1
	sth	%g1, [%fp-386]
	mov	1703, %g1
	sth	%g1, [%fp-384]
	mov	1651, %g1
	sth	%g1, [%fp-382]
	mov	2789, %g1
	sth	%g1, [%fp-380]
	mov	1789, %g1
	sth	%g1, [%fp-378]
	mov	1847, %g1
	sth	%g1, [%fp-376]
	mov	952, %g1
	sth	%g1, [%fp-374]
	mov	1461, %g1
	sth	%g1, [%fp-372]
	mov	2687, %g1
	sth	%g1, [%fp-370]
	mov	939, %g1
	sth	%g1, [%fp-368]
	mov	2308, %g1
	sth	%g1, [%fp-366]
	mov	2437, %g1
	sth	%g1, [%fp-364]
	mov	2388, %g1
	sth	%g1, [%fp-362]
	mov	733, %g1
	sth	%g1, [%fp-360]
	mov	2337, %g1
	sth	%g1, [%fp-358]
	mov	268, %g1
	sth	%g1, [%fp-356]
	mov	641, %g1
	sth	%g1, [%fp-354]
	mov	1584, %g1
	sth	%g1, [%fp-352]
	mov	2298, %g1
	sth	%g1, [%fp-350]
	mov	2037, %g1
	sth	%g1, [%fp-348]
	mov	3220, %g1
	sth	%g1, [%fp-346]
	mov	375, %g1
	sth	%g1, [%fp-344]
	mov	2549, %g1
	sth	%g1, [%fp-342]
	mov	2090, %g1
	sth	%g1, [%fp-340]
	mov	1645, %g1
	sth	%g1, [%fp-338]
	mov	1063, %g1
	sth	%g1, [%fp-336]
	mov	319, %g1
	sth	%g1, [%fp-334]
	mov	2773, %g1
	sth	%g1, [%fp-332]
	mov	757, %g1
	sth	%g1, [%fp-330]
	mov	2099, %g1
	sth	%g1, [%fp-328]
	mov	561, %g1
	sth	%g1, [%fp-326]
	mov	2466, %g1
	sth	%g1, [%fp-324]
	mov	2594, %g1
	sth	%g1, [%fp-322]
	mov	2804, %g1
	sth	%g1, [%fp-320]
	mov	1092, %g1
	sth	%g1, [%fp-318]
	mov	403, %g1
	sth	%g1, [%fp-316]
	mov	1026, %g1
	sth	%g1, [%fp-314]
	mov	1143, %g1
	sth	%g1, [%fp-312]
	mov	2150, %g1
	sth	%g1, [%fp-310]
	mov	2775, %g1
	sth	%g1, [%fp-308]
	mov	886, %g1
	sth	%g1, [%fp-306]
	mov	1722, %g1
	sth	%g1, [%fp-304]
	mov	1212, %g1
	sth	%g1, [%fp-302]
	mov	1874, %g1
	sth	%g1, [%fp-300]
	mov	1029, %g1
	sth	%g1, [%fp-298]
	mov	2110, %g1
	sth	%g1, [%fp-296]
	mov	2935, %g1
	sth	%g1, [%fp-294]
	mov	885, %g1
	sth	%g1, [%fp-292]
	mov	2154, %g1
	sth	%g1, [%fp-290]
	.loc 1 143 0
	sethi	%hi(.LC0), %g1
	or	%g1, %lo(.LC0), %g1
	add	%fp, -1056, %g3
	mov	%g1, %g2
	mov	512, %g1
	mov	%g3, %o0
	mov	%g2, %o1
	mov	%g1, %o2
	call	memcpy, 0
	 nop
	.loc 1 157 0
	sethi	%hi(.LC3), %g1
	or	%g1, %lo(.LC3), %o0
	call	cortos_printf, 0
	 nop
	.loc 1 158 0
	call	__ajit_get_clock_time, 0
	 nop
	std	%o0, [%fp-8]
	.loc 1 159 0
	add	%fp, -1056, %g2
	add	%fp, -544, %g1
	mov	%g2, %o0
	mov	%g1, %o1
	call	ntt_256, 0
	 nop
	.loc 1 160 0
	call	__ajit_get_clock_time, 0
	 nop
	std	%o0, [%fp-16]
	.loc 1 162 0
	sethi	%hi(.LC4), %g1
	or	%g1, %lo(.LC4), %o0
	call	cortos_printf, 0
	 nop
	.loc 1 169 0
	ldd	[%fp-16], %i4
	ldd	[%fp-8], %g2
	subcc	%i5, %g3, %g3
	subx	%i4, %g2, %g2
	.loc 1 168 0
	mov	%g2, %o0
	mov	%g3, %o1
	call	__floatundidf, 0
	 nop
	fmovs	%f0, %f8
	fmovs	%f1, %f9
	sethi	%hi(.LC5), %g1
	or	%g1, %lo(.LC5), %o0
	std	%f8, [%fp-1064]
	ldd	[%fp-1064], %g2
	mov	%g2, %o1
	mov	%g3, %o2
	call	cortos_printf, 0
	 nop
	.loc 1 171 0
	sethi	%hi(.LC6), %g1
	or	%g1, %lo(.LC6), %o0
	call	cortos_printf, 0
	 nop
	.loc 1 173 0
	call	__ajit_get_clock_time, 0
	 nop
	std	%o0, [%fp-24]
	.loc 1 174 0
	add	%fp, -1056, %g2
	add	%fp, -544, %g1
	mov	%g2, %o0
	mov	%g1, %o1
	call	ntt_top, 0
	 nop
	.loc 1 175 0
	call	__ajit_get_clock_time, 0
	 nop
	std	%o0, [%fp-32]
	.loc 1 176 0
	sethi	%hi(.LC7), %g1
	or	%g1, %lo(.LC7), %o0
	call	cortos_printf, 0
	 nop
	.loc 1 179 0
	ldd	[%fp-32], %i4
	ldd	[%fp-24], %g2
	subcc	%i5, %g3, %g3
	subx	%i4, %g2, %g2
	.loc 1 178 0
	mov	%g2, %o0
	mov	%g3, %o1
	call	__floatundidf, 0
	 nop
	fmovs	%f0, %f8
	fmovs	%f1, %f9
	sethi	%hi(.LC8), %g1
	or	%g1, %lo(.LC8), %o0
	std	%f8, [%fp-1064]
	ldd	[%fp-1064], %g2
	mov	%g2, %o1
	mov	%g3, %o2
	call	cortos_printf, 0
	 nop
	.loc 1 180 0
	sethi	%hi(.LC9), %g1
	or	%g1, %lo(.LC9), %o0
	call	cortos_printf, 0
	 nop
	.loc 1 186 0
	sethi	%hi(.LC10), %g1
	or	%g1, %lo(.LC10), %o0
	call	cortos_printf, 0
	 nop
	.loc 1 187 0
	sethi	%hi(tc), %g1
	or	%g1, %lo(tc), %o0
	mov	0, %o1
	mov	0, %o2
	call	scheduleChannelJob, 0
	 nop
	.loc 1 189 0
	mov	1, %g1
	and	%g1, 0xff, %g1
	.loc 1 190 0
	mov	%g1, %i0
	restore
	jmp	%o7+8
	 nop
	.cfi_endproc
.LFE7:
	.size	main_00, .-main_00
	.section	".rodata"
	.align 8
.LC11:
	.asciz	"Entered main_01\n"
	.section	".text"
	.align 4
	.global main_01
	.type	main_01, #function
	.proc	014
main_01:
.LFB8:
	.loc 1 193 0
	.cfi_startproc
	save	%sp, -104, %sp
.LCFI5:
	.cfi_window_save
	.cfi_register 15, 31
	.cfi_def_cfa_register 30
	.loc 1 198 0
	sethi	%hi(.LC11), %g1
	or	%g1, %lo(.LC11), %o0
	call	cortos_printf, 0
	 nop
.L17:
	.loc 1 203 0
	nop
.L12:
	.loc 1 203 0 is_stmt 0 discriminator 1
	add	%fp, -4, %g2
	add	%fp, -8, %g1
	sethi	%hi(tc), %g3
	or	%g3, %lo(tc), %o0
	mov	%g2, %o1
	mov	%g1, %o2
	call	getChannelJob, 0
	 nop
	mov	%o0, %g1
	cmp	%g1, 0
	bne	.L12
	 nop
	.loc 1 207 0 is_stmt 1
	ld	[%fp-4], %g1
	cmp	%g1, 0
	be	.L18
	 nop
	.loc 1 209 0
	ld	[%fp-4], %g1
	ld	[%fp-8], %g2
	mov	%g2, %o0
	call	%g1, 0
	 nop
	.loc 1 210 0
	ld	[%fp-8], %g1
	sethi	%hi(tc), %g2
	or	%g2, %lo(tc), %o0
	mov	%g1, %o1
	call	setChannelResponse, 0
	 nop
	.loc 1 214 0
	nop
	.loc 1 203 0
	b	.L17
	 nop
.L18:
	.loc 1 213 0
	nop
.L16:
	.loc 1 216 0
	mov	1, %g1
	and	%g1, 0xff, %g1
	.loc 1 218 0
	mov	%g1, %i0
	restore
	jmp	%o7+8
	 nop
	.cfi_endproc
.LFE8:
	.size	main_01, .-main_01
.Letext0:
	.file 2 "/home/ajit/ajit-toolchain/build/buildroot-2014.08/output/host/usr/lib/gcc/sparc-buildroot-linux-uclibc/4.7.4/include/stdint-gcc.h"
	.file 3 "/home/ajit/ajit-toolchain/application_development/thread_channel/include/thread_channel.h"
	.section	.debug_info,"",@progbits
.Ldebug_info0:
	.uaword	0x398
	.uahalf	0x2
	.uaword	.Ldebug_abbrev0
	.byte	0x4
	.uleb128 0x1
	.uaword	.LASF39
	.byte	0x1
	.uaword	.LASF40
	.uaword	.LASF41
	.uaword	.Ltext0
	.uaword	.Letext0
	.uaword	.Ldebug_line0
	.uleb128 0x2
	.byte	0x4
	.byte	0x7
	.uaword	.LASF0
	.uleb128 0x2
	.byte	0x1
	.byte	0x8
	.uaword	.LASF1
	.uleb128 0x2
	.byte	0x2
	.byte	0x7
	.uaword	.LASF2
	.uleb128 0x2
	.byte	0x4
	.byte	0x7
	.uaword	.LASF3
	.uleb128 0x2
	.byte	0x1
	.byte	0x6
	.uaword	.LASF4
	.uleb128 0x2
	.byte	0x2
	.byte	0x5
	.uaword	.LASF5
	.uleb128 0x3
	.byte	0x4
	.byte	0x5
	.asciz	"int"
	.uleb128 0x2
	.byte	0x8
	.byte	0x5
	.uaword	.LASF6
	.uleb128 0x2
	.byte	0x8
	.byte	0x7
	.uaword	.LASF7
	.uleb128 0x2
	.byte	0x4
	.byte	0x5
	.uaword	.LASF8
	.uleb128 0x2
	.byte	0x4
	.byte	0x7
	.uaword	.LASF9
	.uleb128 0x4
	.byte	0x4
	.uleb128 0x2
	.byte	0x1
	.byte	0x6
	.uaword	.LASF10
	.uleb128 0x5
	.byte	0x1
	.uaword	0x87
	.uleb128 0x6
	.uaword	0x72
	.byte	0
	.uleb128 0x7
	.byte	0x4
	.uaword	0x7b
	.uleb128 0x8
	.uaword	.LASF11
	.byte	0x2
	.byte	0x2e
	.uaword	0x2c
	.uleb128 0x8
	.uaword	.LASF12
	.byte	0x2
	.byte	0x31
	.uaword	0x33
	.uleb128 0x8
	.uaword	.LASF13
	.byte	0x2
	.byte	0x34
	.uaword	0x25
	.uleb128 0x8
	.uaword	.LASF14
	.byte	0x2
	.byte	0x37
	.uaword	0x5d
	.uleb128 0x2
	.byte	0x4
	.byte	0x4
	.uaword	.LASF15
	.uleb128 0x2
	.byte	0x8
	.byte	0x4
	.uaword	.LASF16
	.uleb128 0x9
	.uaword	.LASF23
	.byte	0x18
	.byte	0x3
	.byte	0x1f
	.uaword	0x127
	.uleb128 0xa
	.asciz	"id"
	.byte	0x3
	.byte	0x21
	.uaword	0xa3
	.byte	0x2
	.byte	0x23
	.uleb128 0
	.uleb128 0xb
	.uaword	.LASF17
	.byte	0x3
	.byte	0x22
	.uaword	0xa3
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.uleb128 0xb
	.uaword	.LASF18
	.byte	0x3
	.byte	0x23
	.uaword	0xa3
	.byte	0x2
	.byte	0x23
	.uleb128 0x8
	.uleb128 0xb
	.uaword	.LASF19
	.byte	0x3
	.byte	0x24
	.uaword	0x72
	.byte	0x2
	.byte	0x23
	.uleb128 0xc
	.uleb128 0xb
	.uaword	.LASF20
	.byte	0x3
	.byte	0x25
	.uaword	0x72
	.byte	0x2
	.byte	0x23
	.uleb128 0x10
	.uleb128 0xb
	.uaword	.LASF21
	.byte	0x3
	.byte	0x28
	.uaword	0x4f
	.byte	0x2
	.byte	0x23
	.uleb128 0x14
	.byte	0
	.uleb128 0x8
	.uaword	.LASF22
	.byte	0x3
	.byte	0x2a
	.uaword	0xc7
	.uleb128 0x9
	.uaword	.LASF24
	.byte	0x8
	.byte	0x1
	.byte	0x15
	.uaword	0x159
	.uleb128 0xa
	.asciz	"x"
	.byte	0x1
	.byte	0x17
	.uaword	0x159
	.byte	0x2
	.byte	0x23
	.uleb128 0
	.uleb128 0xb
	.uaword	.LASF25
	.byte	0x1
	.byte	0x18
	.uaword	0x159
	.byte	0x2
	.byte	0x23
	.uleb128 0x4
	.byte	0
	.uleb128 0x7
	.byte	0x4
	.uaword	0x98
	.uleb128 0x8
	.uaword	.LASF24
	.byte	0x1
	.byte	0x1a
	.uaword	0x132
	.uleb128 0xc
	.byte	0x1
	.uaword	.LASF27
	.byte	0x1
	.byte	0x1c
	.byte	0x1
	.uaword	.LFB3
	.uaword	.LFE3
	.uaword	.LLST0
	.byte	0x1
	.uaword	0x1b0
	.uleb128 0xd
	.uaword	.LASF26
	.byte	0x1
	.byte	0x1c
	.uaword	0x1b0
	.byte	0x3
	.byte	0x91
	.sleb128 68
	.uleb128 0xe
	.asciz	"x"
	.byte	0x1
	.byte	0x1c
	.uaword	0x159
	.byte	0x3
	.byte	0x91
	.sleb128 72
	.uleb128 0xd
	.uaword	.LASF25
	.byte	0x1
	.byte	0x1c
	.uaword	0x159
	.byte	0x3
	.byte	0x91
	.sleb128 76
	.byte	0
	.uleb128 0x7
	.byte	0x4
	.uaword	0x15f
	.uleb128 0xf
	.byte	0x1
	.uaword	.LASF28
	.byte	0x1
	.byte	0x23
	.byte	0x1
	.uaword	.LFB4
	.uaword	.LFE4
	.uaword	.LLST1
	.byte	0x1
	.uaword	0x203
	.uleb128 0xd
	.uaword	.LASF26
	.byte	0x1
	.byte	0x23
	.uaword	0x72
	.byte	0x3
	.byte	0x91
	.sleb128 68
	.uleb128 0x10
	.asciz	"x"
	.byte	0x1
	.byte	0x24
	.uaword	0x159
	.byte	0x2
	.byte	0x91
	.sleb128 -4
	.uleb128 0x11
	.uaword	.LASF25
	.byte	0x1
	.byte	0x25
	.uaword	0x159
	.byte	0x2
	.byte	0x91
	.sleb128 -8
	.uleb128 0x12
	.asciz	"i"
	.byte	0x1
	.byte	0x26
	.uaword	0x98
	.byte	0
	.uleb128 0xf
	.byte	0x1
	.uaword	.LASF29
	.byte	0x1
	.byte	0x35
	.byte	0x1
	.uaword	.LFB5
	.uaword	.LFE5
	.uaword	.LLST2
	.byte	0x1
	.uaword	0x28d
	.uleb128 0xe
	.asciz	"x"
	.byte	0x1
	.byte	0x35
	.uaword	0x159
	.byte	0x3
	.byte	0x91
	.sleb128 68
	.uleb128 0xd
	.uaword	.LASF25
	.byte	0x1
	.byte	0x35
	.uaword	0x159
	.byte	0x3
	.byte	0x91
	.sleb128 72
	.uleb128 0x11
	.uaword	.LASF30
	.byte	0x1
	.byte	0x37
	.uaword	0x28d
	.byte	0x2
	.byte	0x91
	.sleb128 -8
	.uleb128 0x11
	.uaword	.LASF31
	.byte	0x1
	.byte	0x38
	.uaword	0x29d
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.uleb128 0x10
	.asciz	"i"
	.byte	0x1
	.byte	0x39
	.uaword	0x8d
	.byte	0x2
	.byte	0x91
	.sleb128 -1
	.uleb128 0x10
	.asciz	"xe"
	.byte	0x1
	.byte	0x3a
	.uaword	0x2a2
	.byte	0x3
	.byte	0x91
	.sleb128 -272
	.uleb128 0x10
	.asciz	"xo"
	.byte	0x1
	.byte	0x3a
	.uaword	0x2a2
	.byte	0x3
	.byte	0x91
	.sleb128 -528
	.uleb128 0x11
	.uaword	.LASF32
	.byte	0x1
	.byte	0x68
	.uaword	0x72
	.byte	0x3
	.byte	0x91
	.sleb128 -532
	.byte	0
	.uleb128 0x13
	.uaword	0x292
	.uleb128 0x7
	.byte	0x4
	.uaword	0x298
	.uleb128 0x13
	.uaword	0x127
	.uleb128 0x13
	.uaword	0x15f
	.uleb128 0x14
	.uaword	0x98
	.uaword	0x2b2
	.uleb128 0x15
	.uaword	0x6b
	.byte	0x7f
	.byte	0
	.uleb128 0x16
	.byte	0x1
	.uaword	.LASF42
	.byte	0x1
	.byte	0x72
	.uaword	.LFB6
	.uaword	.LFE6
	.uaword	.LLST3
	.byte	0x1
	.uleb128 0x17
	.byte	0x1
	.uaword	.LASF33
	.byte	0x1
	.byte	0x7f
	.uaword	0x8d
	.uaword	.LFB7
	.uaword	.LFE7
	.uaword	.LLST4
	.byte	0x1
	.uaword	0x340
	.uleb128 0x12
	.asciz	"i"
	.byte	0x1
	.byte	0x82
	.uaword	0x8d
	.uleb128 0x11
	.uaword	.LASF25
	.byte	0x1
	.byte	0x84
	.uaword	0x340
	.byte	0x3
	.byte	0x91
	.sleb128 -544
	.uleb128 0x10
	.asciz	"x"
	.byte	0x1
	.byte	0x8f
	.uaword	0x340
	.byte	0x3
	.byte	0x91
	.sleb128 -1056
	.uleb128 0x11
	.uaword	.LASF34
	.byte	0x1
	.byte	0x9e
	.uaword	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -8
	.uleb128 0x11
	.uaword	.LASF35
	.byte	0x1
	.byte	0xa0
	.uaword	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -16
	.uleb128 0x10
	.asciz	"t0"
	.byte	0x1
	.byte	0xad
	.uaword	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -24
	.uleb128 0x10
	.asciz	"t1"
	.byte	0x1
	.byte	0xaf
	.uaword	0xae
	.byte	0x2
	.byte	0x91
	.sleb128 -32
	.byte	0
	.uleb128 0x14
	.uaword	0x98
	.uaword	0x350
	.uleb128 0x15
	.uaword	0x6b
	.byte	0xff
	.byte	0
	.uleb128 0x17
	.byte	0x1
	.uaword	.LASF36
	.byte	0x1
	.byte	0xc0
	.uaword	0x8d
	.uaword	.LFB8
	.uaword	.LFE8
	.uaword	.LLST5
	.byte	0x1
	.uaword	0x38a
	.uleb128 0x11
	.uaword	.LASF37
	.byte	0x1
	.byte	0xc2
	.uaword	0x87
	.byte	0x2
	.byte	0x91
	.sleb128 -4
	.uleb128 0x11
	.uaword	.LASF38
	.byte	0x1
	.byte	0xc3
	.uaword	0x72
	.byte	0x2
	.byte	0x91
	.sleb128 -8
	.byte	0
	.uleb128 0x18
	.asciz	"tc"
	.byte	0x1
	.byte	0x13
	.uaword	0x298
	.byte	0x1
	.byte	0x5
	.byte	0x3
	.uaword	tc
	.byte	0
	.section	.debug_abbrev,"",@progbits
.Ldebug_abbrev0:
	.uleb128 0x1
	.uleb128 0x11
	.byte	0x1
	.uleb128 0x25
	.uleb128 0xe
	.uleb128 0x13
	.uleb128 0xb
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x1b
	.uleb128 0xe
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x10
	.uleb128 0x6
	.byte	0
	.byte	0
	.uleb128 0x2
	.uleb128 0x24
	.byte	0
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3e
	.uleb128 0xb
	.uleb128 0x3
	.uleb128 0xe
	.byte	0
	.byte	0
	.uleb128 0x3
	.uleb128 0x24
	.byte	0
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3e
	.uleb128 0xb
	.uleb128 0x3
	.uleb128 0x8
	.byte	0
	.byte	0
	.uleb128 0x4
	.uleb128 0xf
	.byte	0
	.uleb128 0xb
	.uleb128 0xb
	.byte	0
	.byte	0
	.uleb128 0x5
	.uleb128 0x15
	.byte	0x1
	.uleb128 0x27
	.uleb128 0xc
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x6
	.uleb128 0x5
	.byte	0
	.uleb128 0x49
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x7
	.uleb128 0xf
	.byte	0
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x8
	.uleb128 0x16
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x9
	.uleb128 0x13
	.byte	0x1
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0xb
	.uleb128 0xb
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0xa
	.uleb128 0xd
	.byte	0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x38
	.uleb128 0xa
	.byte	0
	.byte	0
	.uleb128 0xb
	.uleb128 0xd
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x38
	.uleb128 0xa
	.byte	0
	.byte	0
	.uleb128 0xc
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x3f
	.uleb128 0xc
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0xc
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x40
	.uleb128 0x6
	.uleb128 0x2117
	.uleb128 0xc
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0xd
	.uleb128 0x5
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0xa
	.byte	0
	.byte	0
	.uleb128 0xe
	.uleb128 0x5
	.byte	0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0xa
	.byte	0
	.byte	0
	.uleb128 0xf
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x3f
	.uleb128 0xc
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x27
	.uleb128 0xc
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x40
	.uleb128 0x6
	.uleb128 0x2116
	.uleb128 0xc
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x10
	.uleb128 0x34
	.byte	0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0xa
	.byte	0
	.byte	0
	.uleb128 0x11
	.uleb128 0x34
	.byte	0
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2
	.uleb128 0xa
	.byte	0
	.byte	0
	.uleb128 0x12
	.uleb128 0x34
	.byte	0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x13
	.uleb128 0x35
	.byte	0
	.uleb128 0x49
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x14
	.uleb128 0x1
	.byte	0x1
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x15
	.uleb128 0x21
	.byte	0
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x2f
	.uleb128 0xb
	.byte	0
	.byte	0
	.uleb128 0x16
	.uleb128 0x2e
	.byte	0
	.uleb128 0x3f
	.uleb128 0xc
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x40
	.uleb128 0x6
	.uleb128 0x2116
	.uleb128 0xc
	.byte	0
	.byte	0
	.uleb128 0x17
	.uleb128 0x2e
	.byte	0x1
	.uleb128 0x3f
	.uleb128 0xc
	.uleb128 0x3
	.uleb128 0xe
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x11
	.uleb128 0x1
	.uleb128 0x12
	.uleb128 0x1
	.uleb128 0x40
	.uleb128 0x6
	.uleb128 0x2116
	.uleb128 0xc
	.uleb128 0x1
	.uleb128 0x13
	.byte	0
	.byte	0
	.uleb128 0x18
	.uleb128 0x34
	.byte	0
	.uleb128 0x3
	.uleb128 0x8
	.uleb128 0x3a
	.uleb128 0xb
	.uleb128 0x3b
	.uleb128 0xb
	.uleb128 0x49
	.uleb128 0x13
	.uleb128 0x3f
	.uleb128 0xc
	.uleb128 0x2
	.uleb128 0xa
	.byte	0
	.byte	0
	.byte	0
	.section	.debug_loc,"",@progbits
.Ldebug_loc0:
.LLST0:
	.uaword	.LFB3-.Ltext0
	.uaword	.LCFI0-.Ltext0
	.uahalf	0x2
	.byte	0x7e
	.sleb128 0
	.uaword	.LCFI0-.Ltext0
	.uaword	.LFE3-.Ltext0
	.uahalf	0x2
	.byte	0x8e
	.sleb128 0
	.uaword	0
	.uaword	0
.LLST1:
	.uaword	.LFB4-.Ltext0
	.uaword	.LCFI1-.Ltext0
	.uahalf	0x2
	.byte	0x7e
	.sleb128 0
	.uaword	.LCFI1-.Ltext0
	.uaword	.LFE4-.Ltext0
	.uahalf	0x2
	.byte	0x8e
	.sleb128 0
	.uaword	0
	.uaword	0
.LLST2:
	.uaword	.LFB5-.Ltext0
	.uaword	.LCFI2-.Ltext0
	.uahalf	0x2
	.byte	0x7e
	.sleb128 0
	.uaword	.LCFI2-.Ltext0
	.uaword	.LFE5-.Ltext0
	.uahalf	0x2
	.byte	0x8e
	.sleb128 0
	.uaword	0
	.uaword	0
.LLST3:
	.uaword	.LFB6-.Ltext0
	.uaword	.LCFI3-.Ltext0
	.uahalf	0x2
	.byte	0x7e
	.sleb128 0
	.uaword	.LCFI3-.Ltext0
	.uaword	.LFE6-.Ltext0
	.uahalf	0x2
	.byte	0x8e
	.sleb128 0
	.uaword	0
	.uaword	0
.LLST4:
	.uaword	.LFB7-.Ltext0
	.uaword	.LCFI4-.Ltext0
	.uahalf	0x2
	.byte	0x7e
	.sleb128 0
	.uaword	.LCFI4-.Ltext0
	.uaword	.LFE7-.Ltext0
	.uahalf	0x2
	.byte	0x8e
	.sleb128 0
	.uaword	0
	.uaword	0
.LLST5:
	.uaword	.LFB8-.Ltext0
	.uaword	.LCFI5-.Ltext0
	.uahalf	0x2
	.byte	0x7e
	.sleb128 0
	.uaword	.LCFI5-.Ltext0
	.uaword	.LFE8-.Ltext0
	.uahalf	0x2
	.byte	0x8e
	.sleb128 0
	.uaword	0
	.uaword	0
	.section	.debug_aranges,"",@progbits
	.uaword	0x1c
	.uahalf	0x2
	.uaword	.Ldebug_info0
	.byte	0x4
	.byte	0
	.uahalf	0
	.uahalf	0
	.uaword	.Ltext0
	.uaword	.Letext0-.Ltext0
	.uaword	0
	.uaword	0
	.section	.debug_line,"",@progbits
.Ldebug_line0:
	.section	.debug_str,"MS",@progbits,1
.LASF26:
	.asciz	"args"
.LASF31:
	.asciz	"ntt_args"
.LASF16:
	.asciz	"double"
.LASF14:
	.asciz	"uint64_t"
.LASF21:
	.asciz	"status"
.LASF23:
	.asciz	"ThreadChannel__"
.LASF37:
	.asciz	"__fn"
.LASF22:
	.asciz	"ThreadChannel"
.LASF15:
	.asciz	"float"
.LASF36:
	.asciz	"main_01"
.LASF19:
	.asciz	"fn_ptr"
.LASF32:
	.asciz	"vptr"
.LASF3:
	.asciz	"long unsigned int"
.LASF2:
	.asciz	"short unsigned int"
.LASF34:
	.asciz	"t00_0"
.LASF35:
	.asciz	"t00_1"
.LASF25:
	.asciz	"psis"
.LASF28:
	.asciz	"ntt_thread"
.LASF17:
	.asciz	"request_id"
.LASF1:
	.asciz	"unsigned char"
.LASF33:
	.asciz	"main_00"
.LASF5:
	.asciz	"short int"
.LASF0:
	.asciz	"unsigned int"
.LASF7:
	.asciz	"long long unsigned int"
.LASF11:
	.asciz	"uint8_t"
.LASF42:
	.asciz	"setup"
.LASF29:
	.asciz	"ntt_top"
.LASF38:
	.asciz	"__arg"
.LASF9:
	.asciz	"sizetype"
.LASF27:
	.asciz	"set_NTT_Args"
.LASF6:
	.asciz	"long long int"
.LASF10:
	.asciz	"char"
.LASF18:
	.asciz	"response_id"
.LASF40:
	.asciz	"../main.c"
.LASF39:
	.asciz	"GNU C 4.7.4"
.LASF24:
	.asciz	"thread_args"
.LASF41:
	.asciz	"/Satvik/fft/cortos_build"
.LASF12:
	.asciz	"uint16_t"
.LASF13:
	.asciz	"uint32_t"
.LASF8:
	.asciz	"long int"
.LASF30:
	.asciz	"tc_ptr"
.LASF4:
	.asciz	"signed char"
.LASF20:
	.asciz	"arg_ptr"
	.ident	"GCC: (Buildroot 2014.08) 4.7.4"
	.section	.note.GNU-stack,"",@progbits
